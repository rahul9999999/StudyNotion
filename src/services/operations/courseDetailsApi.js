...
export const addCourseDetails = async (data, token) => {
  ...
  try {
    ...
    result = response?.data?.CourseCreating;
  } catch (error) {
...
export const createSubSection = async (data, token) => {
  console.log("createSubSectionData",data)
  let result = null;
...
export const getCategoryId = async (data, token) => {
  // const toastId = toast.loading("Loading...");
  let success ;
  try {
    const response = await apiConnector("POST", CATEGORIES_GET, data, {
      Authorisation: `Bearer ${token}`,
    });
    console.log("GET CATEGORY ID API RESPONSE............", response);
    if (!response?.data?.success) {
      throw new Error("Could Not Find CategoryId");
    }
    
    success = response?.data?.result
  } catch (error) {
    console.log("GET CATEGORY ID API ERROR............", error);
    toast.error(error.message);
  }
  // toast.dismiss(toastId);
  return success;
