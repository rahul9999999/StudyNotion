...
const EnrolledCourses = () => {
    ...
    const getEnrolledCourses = async () => {
        try {
            ...
            setProgressData(response?.coursesInProgress);
            setLoading(false);
        } catch (error) {
            setLoading(false)
            console.log("Error while  fetching entrolled courses")
            console.log(error)
        }
    }
                                    ...
                                    enrolledCourses.map((course, index) => (
                                        <div key={index} onClick={() => {
                                            ...
                                            <div className='flex w-[45%] cursor-pointer items-center gap-4 px-5 py-3'>
                                                <img className='h-14 w-14 rounded-lg object-cover' src={course?.thumbnail?.url} />
                                            ...
                                            <div className='flex w-1/5 flex-col gap-2 px- py-3'>
                                                {progressData.length === 0 ?
                                                    <div>
                                                        <p>Completed: {0} / {totalNoOfLectures(course)}</p>
                                                        <ProgressBar
                                                            completed={0 / totalNoOfLectures(course) * 100}
                                                            height='8px'
