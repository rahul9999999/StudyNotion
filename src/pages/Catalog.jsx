...
import { CardPlacehoderSkeleton } from '../components/common/Skeleton';
...
const Catalog = () => {
  ...
  const fetchSublinks = async () => {
    try {
      ...
      const category_id = result?.data.category.filter((item) => item.name === Catalog.catalogName)[0]._id;
      setcategoryID(category_id);
      setDesc(result?.data?.category.filter((item) => item.name === Catalog.catalogName)[0]);
  ...
  return (
    <div>
      ...
      <div className=' mx-auto box-content w-full max-w-maxContentTab px-2 py-12 lg:max-w-maxContent'>
        ...
        <CourseSlider Courses={CatalogPageData?.specificCategory?.courses} />
      </div>
      ...
      <div className=' mx-auto box-content w-full max-w-maxContentTab px-2 py-12 lg:max-w-maxContent'>
        ...
        {CatalogPageData?.mostSellingCourses ? <div className='grid grid-cols-2 gap-3 lg:gap-6 lg:grid-cols-2 pr-4'>
          {
